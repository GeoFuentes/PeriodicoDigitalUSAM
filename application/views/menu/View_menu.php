            ...
            <form class="" id="developer_cu_form">
                <div class="modal-body">
                    <div class="input-group">
                        ...
                        <div class="form-line">
                            <input type="text" class="form-control date" placeholder="Menu" id="menu2" name="menu">
                        </div>
                    </div>
                    <div class="input-group">
                        <span class="input-group-addon">
                        ...
                        <div class="form-line">
                            <select name="icono" id="icono"  class="form-control date se" data-live-search="true" style="width: 100%;">
...
<script type="text/javascript">
    ...
    $(document).on("click", "#delteBtnId", function (e) {
        ...
        swal({
            ...
            if (isConfirm) {
                var action = 'delete';
                $.ajax({
                    url: "<?php echo base_url(); ?>Controller_menu/eliminar_menu",
                    method: "POST",
                    data: {
    ...
    $(document).on("click", "#editBtnId", function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>Controller_menu/linea_actualizar",
            method: "POST",
            data: {
                editBtnId: editBtnId,
                action: action
            ...
            success: function (data) {
                ...
                $("#icono").val(data.icono);
                 $("#icono").change();
                $("#select2-icono-container").html("<span class='" + data.icono + "'>" + data.icono + "</span>");
                $("#form-title").text('Editar menu');
                $("#action").val('update');
    ...
    $(document).on("submit", "#developer_cu_form", function (e) {
        ...
        if (menu == '') {
            swal({
                title: "Campo menu requerido",
                type: "warning"
            });
        } else {
            $.ajax({
                url: "<?php echo base_url(); ?>Controller_menu/actualizar_crear_menu",
                method: "POST",
